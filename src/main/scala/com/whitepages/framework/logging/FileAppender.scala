...
private[logging] class FileAppenderActor(path: String, category: String) extends Actor with ActorSupport {
...
private[logging] case class FileAppender(actorRefFactory: ActorRefFactory, path: String, category: String) {
  import FileAppender._

  private[this] val fileAppenderActor = actorRefFactory.actorOf(Props(classOf[FileAppenderActor], path, category), name = s"Appender.$category")
