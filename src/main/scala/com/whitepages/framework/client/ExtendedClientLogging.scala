...
import com.whitepages.framework.logging.{DurationStrings, RequestId, AnyId}
...
private[client] case class ExtendedClientLogging(mapper: ExtendedClientLogging.Mapper, clientName: String) extends ClassSupport {
  ...
  private[client] def logIt(duration: Long,
    ...
    val props1 = JsonObject("duration" -> DurationStrings.microsDuration(duration/1000), "@traceId" -> traceId, "client" -> clientName)
